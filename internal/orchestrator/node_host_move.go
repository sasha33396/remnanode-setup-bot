...
// MoveNodeToHost re-reads both the Node and Host before PATCH. It updates only
// Remnawave's configProfile binding; the Node address is never modified.
func (s *DeploymentService) MoveNodeToHost(ctx context.Context, input NodeHostMoveInput) (NodeHostMoveResult, error) {
	...
	if input.NodeUUID == "" || input.TargetHostUUID == "" {
	...
	managed := false
	if _, managedErr := s.repository.FindDeploymentByPanelNodeUUID(runCtx, s.config.PanelID, node.UUID); managedErr == nil {
		managed = true
	} else if !errors.Is(managedErr, repository.ErrNotFound) {
